...
`@tts-open-toolkit/cli` provides the `tts_open_toolkit` command. It installs and updates TTS coding-agent skills, manages Partner Center OAuth profiles, calls fixed OAuth DevAPI methods, exposes sandbox operations, configures Codex / Claude Code hooks, records skill usage history, generates shell completion, and checks for toolkit updates.
...
| Install or update TTS skills | `skill add`, `skill list`, `skill state` | Choose project, user, agent, or explicit target. Use `--update` for managed installs; state ignores skills installed by other tools. |
...
| Inspect apps or sandbox state | `app list`, `sandbox shop list`, `sandbox endpoints` | Sandbox account deletion is intentionally unavailable. |
| Create and transition BSO sandbox orders | `sandbox product list`, `sandbox order create`, `sandbox order list`, `sandbox order transition` | Typed commands default to BSO v3 and validate nested business results. |
| Configure host hooks or inspect usage | `setup`, `history` | Use dry-run before writing host settings; history remains local. |
| Enable Tab completion | `completion bash|zsh|fish` | Load the generated script from the matching shell startup file. |
| Check or install an update | `update channel`, `update --check`, `update --yes` | Confirm the user channel first. Check is read-only; `--yes` is required before package installation. |
...
5. For ongoing maintenance, confirm the user update channel, then use `tts_open_toolkit update --check` and `tts_open_toolkit update --yes`.
6. For OAuth-backed account, app, service, document, and authorization operations, inspect `tts_open_toolkit devapi list --json` and use `devapi call <method-name>` instead of a caller-provided route.
7. For BSO sandbox order workflows, use the typed `sandbox shop/product/order` commands instead of constructing a raw body through `sandbox call`.
8. Use `auth request` only for explicit diagnostics against a known existing TTSPC route that is not represented by a fixed method; never use it to bypass an API allowlist.
9. For broader developer-shop, app, or API questions, pair this skill with `tts-openapi-guide` or `tts-developer-onboarding-guide`.
...
tts_open_toolkit sandbox order create --shop-id <shop-id> --item <product-id>:<sku-id>:1 --order-count 1 --logistics-service-id <logistics-service-id> --payment-method 1 --json
tts_open_toolkit sandbox order list --shop-id <shop-id> --json
tts_open_toolkit sandbox order transition <order-id> --shop-id <shop-id> --json
...
The user update channel defaults to `beta`. Beta follows Beta, RC, and Stable releases; RC follows RC and Stable; Stable follows only Stable. Project configuration cannot change this user preference.
...
- Prefer typed `sandbox shop/product/order` commands over raw BSO endpoint calls. Treat an unchanged status after `sandbox order transition` as a failed transition even when the status endpoint returns success.
