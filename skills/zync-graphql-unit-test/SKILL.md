...
- **The `use<Feature>Query()` hook** (usually in `./gql/query.ts`, sometimes inlined directly in `context.tsx`) → open it and check **how each operation is returned**, because it varies per module:
  ...
  - Named object of raw tuples (a third variant, seen in msgd-staff `payment`/`scheme`): `return { paymentMethods: useLazyQuery(...) }` (no `[0]` unwrap) → mock as `{ paymentMethods: [jest.fn(), {}] }`, called as `queryResult.paymentMethods[0](...)`.
...
| Calling a context method | Wrap in `act(async () => { await result.current.methodName(...) })` — state updates from async setState calls outside `act()` produce "not wrapped in act" warnings and can read stale state. On React 19 + `@testing-library/react-native` (msgd-staff, msgld-app), this is not optional even for a *synchronous* state setter (e.g. `selectLoan(id)`): a plain `act(() => { result.current.selectLoan(id) })` can leave `result.current` stale (even `null`) for every *subsequent* test in the same file. Always use `await act(async () => { ...call...; await Promise.resolve(); })`, never the sync form, on this stack. |
| Asserting "this call doesn't throw" | Don't write `expect(act(...)).resolves.not.toThrow()` — `act()`'s return value is `undefined` regardless of whether the callback threw, so `.not.toThrow()` on it asserts nothing. Wrap the call in `try/catch` inside the `act()` callback and assert on the caught error (or its absence) directly. |
